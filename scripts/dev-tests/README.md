...
| `public-web.js` | **Smerovanie verejného webu** medzi plaw.win / plaw.online / plaw.click — každá stránka odpovedá 200 práve na jednom hostiteľovi, jazyk prežije skok na druhú doménu a appkové cesty na marketingových doménach nežijú  a §5b **text pre rodičov** — celý, bez návodu pre trénera a interných poznámok, s voliteľným odsekom v ráme a so zvýraznenými miestami, ktoré si dopĺňa tréner |
...
| `drill-code-prefixes.js` | **Predvolené kódy cvičení v konfigurácii disciplín** — každý kód v zoskupenom zameraní (Return, Serve) sedí na svoj prefix a nespadne ticho do „Other", stĺpce na `/drill-codes` sedia so skupinami v analytike, žiadne duplicity, podozrivé znaky ani viac než 20 kódov, zamerania bez lomky. Obsahuje kontrolný pokus s podvrhnutým kódom. Číta `lib/disciplines/*.ts` priamo, **nepotrebuje dev server ani databázu** — spúšťaj ho pri každej zmene predvolených kódov |
